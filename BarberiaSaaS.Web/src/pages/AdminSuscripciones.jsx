...
export default function AdminSuscripciones() {
  ...
  const pendientes = negocios.filter(x => x.emailConfirmado === false);
  const confirmados = negocios.filter(x => x.emailConfirmado !== false);

  const cambiar = (id, campo, valor) => setNegocios(actual => actual.map(x => x.id === id ? { ...x, [campo]: valor } : x));

  const prepararMes = (negocio) => {
    setMensaje(""); setError("");
    setNegocios(actual => actual.map(x => x.id === negocio.id ? {
      ...x, accesoCortesia: false, metodoSuscripcion: x.metodoSuscripcion || "SINPE", fechaManual: sumarUnMes(x.fechaManual)
    } : x));
  };
  ...
  const guardar = async (negocio) => {
    ...
    if (!negocio.accesoCortesia && negocio.metodoSuscripcion && !negocio.fechaManual) {
      setError(`Selecciona la fecha de vencimiento para ${negocio.nombre}.`); return;
    }
    try {
      setGuardandoId(negocio.id);
      const suscripcionHasta = negocio.accesoCortesia || !negocio.fechaManual ? null : new Date(`${negocio.fechaManual}T23:59:59`).toISOString();
  ...
  const eliminarPendiente = async (negocio) => {
    ...
    try {
      ...
      setMensaje(`Registro pendiente eliminado: ${negocio.nombre}`);
      await cargar();
    } catch (err) {
      setError(err?.response?.data?.message || err?.response?.data?.mensaje || `No se pudo eliminar ${negocio.nombre}.`);
  ...
  return (
    <div className="container-fluid px-0">
      <div className="mb-4">
        ...
        <p className="text-muted mb-0">Controla PayPal, SINPE, efectivo, cortesías y registros pendientes.</p>
      </div>
      {mensaje && <div className="alert alert-success">{mensaje}</div>}
      {error && <div className="alert alert-danger">{error}</div>}

      <div className="card border-warning shadow-sm mb-4" style={{ borderRadius: 18 }}>
        <div className="card-body p-4">
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
            <div><h5 className="fw-bold mb-1">Registros pendientes</h5><div className="text-muted small">Negocios que todavía no han confirmado su correo. Solo estos registros pueden eliminarse aquí.</div></div>
            <span className="badge text-bg-warning">{pendientes.length} pendiente{pendientes.length === 1 ? "" : "s"}</span>
          </div>
          {pendientes.length === 0 ? <div className="text-muted">No hay registros pendientes 🎉</div> : (
            <div className="table-responsive">
              <table className="table align-middle mb-0">
                <thead><tr><th>Negocio</th><th>Correo</th><th>Creado</th><th className="text-end">Acción</th></tr></thead>
                <tbody>{pendientes.map(n => (
                  <tr key={n.id}>
                    <td><div className="fw-semibold">{n.nombre}</div><div className="small text-muted">Tenant #{n.id}</div></td>
                    <td>{n.email || "—"}</td><td>{fechaBonita(n.fechaCreacion)}</td>
                    <td className="text-end"><button className="btn btn-outline-danger btn-sm fw-semibold" disabled={eliminandoId === n.id} onClick={() => eliminarPendiente(n)}><FaTrashAlt className="me-2" />{eliminandoId === n.id ? "Eliminando..." : "Eliminar registro"}</button></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
          ...
          <div key={n.id} className="card border-0 shadow-sm" style={{ borderRadius: 18 }}>
            <div className="card-body p-4">
              ...
              <div className="d-flex flex-wrap justify-content-end gap-2 mt-3">
                {!n.accesoCortesia && <button className="btn btn-outline-primary fw-semibold" onClick={() => prepararMes(n)}><FaCalendarPlus className="me-2" />+1 mes</button>}
                <button className="btn btn-primary fw-semibold" disabled={guardandoId === n.id} onClick={() => guardar(n)}><FaSave className="me-2" />{guardandoId === n.id ? "Guardando..." : "Guardar acceso"}</button>
              </div>
            </div>
          </div>
        ))}
