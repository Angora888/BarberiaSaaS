...
function Clientes() {
  ...
  const plataformaContacto = useMemo(() => {
    if (typeof navigator === "undefined") return "otro";

    const plataforma = navigator.userAgentData?.platform || "";
    const userAgent = navigator.userAgent || "";

    if (/android/i.test(plataforma) || /android/i.test(userAgent)) {
      return "android";
    }

    if (/ios/i.test(plataforma) || /iphone|ipad|ipod/i.test(userAgent)) {
      return "iphone";
    }
  ...
  const textoImportacion =
    plataformaContacto === "android"
      ? {
          boton: "Importar contacto de Android",
          ayuda:
            "En Contactos de Android: abre el contacto, toca Compartir o Exportar, guarda el archivo .vcf y luego selecciónalo aquí."
        }
      : plataformaContacto === "iphone"
        ? {
            boton: "Importar contacto del iPhone",
            ayuda:
              "En Contactos del iPhone: abre el contacto, toca Compartir contacto, guarda el archivo en Archivos y luego selecciónalo aquí."
          }
        : {
            boton: "Importar contacto (.vcf)",
            ayuda:
              "Importa un contacto exportado desde iPhone o Android en formato .vcf. Selecciona el archivo y revisa los datos antes de guardar."
          };
  ...
  const limpiarFormulario = () => setFormulario(formularioVacio());
  ...
  const cerrarFormulario = () => {
    ...
    if (inputVcardRef.current) inputVcardRef.current.value = "";
  };
  ...
  const unirLineasVcard = (contenido) => {
    ...
    for (const linea of lineas) {
      if ((linea.startsWith(" ") || linea.startsWith("\t")) && resultado.length > 0) {
  ...
  const parsearNombreVcard = (lineas) => {
    ...
    if (nombreEstructurado) {
      const partes = nombreEstructurado.split(";").map((parte) => decodificarTextoVcard(parte));
      const apellidos = [partes[0]].filter(Boolean).join(" ").trim();
      const nombre = [partes[1], partes[2]].filter(Boolean).join(" ").trim();
      if (nombre || apellidos) return { nombre: nombre || nombreCompleto || "", apellidos };
    }
  ...
  const importarVcard = async (e) => {
    ...
    try {
      ...
      const inicio = lineas.findIndex((linea) => linea.trim().toUpperCase() === "BEGIN:VCARD");
      const fin = lineas.findIndex(
        (linea, indice) => indice > inicio && linea.trim().toUpperCase() === "END:VCARD"
      );

      if (inicio < 0 || fin < 0) {
        throw new Error("El archivo seleccionado no parece ser un contacto válido.");
      }
      ...
      if (!nombre && !telefono && !email) {
        throw new Error("No fue posible encontrar nombre, teléfono o correo en el contacto.");
      }
      ...
      setError(error.message || "No fue posible importar el contacto.");
    } finally {
      if (inputVcardRef.current) inputVcardRef.current.value = "";
    }
  };
  ...
  const guardarCliente = async (e) => {
    ...
    try {
      ...
      setError("");
      setMensaje("");
      if (clienteEditando) await api.put(`/Clientes/${clienteEditando.id}`, payload);
      ...
      setError(
        error.response?.data?.mensaje ||
          (clienteEditando ? "No fue posible actualizar el cliente." : "No fue posible crear el cliente.")
      );
    } finally {
      setGuardando(false);
    }
  ...
  const abrirHistorial = async (clienteId) => {
    try {
      ...
      setError(error.response?.data?.mensaje || "No fue posible cargar el historial.");
    } finally {
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="text-muted mb-0">Administra los clientes del negocio.</p>
        </div>
        ...
        {cargando ? (
          ...
          <div className="table-responsive">
            <table className="table align-middle client-table">
              <thead>
                <tr>
                  <th>Cliente</th><th>Teléfono</th><th>Correo</th><th>Fecha nacimiento</th><th>Deuda</th><th></th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={cliente.id}>
                      <td>
                        <div className="client-name-cell">
                          <div className="client-avatar">{cliente.nombre?.charAt(0).toUpperCase()}</div>
                          <div>
                            <strong>{cliente.nombre} {cliente.apellidos}</strong>
                            {cliente.notas && <div className="client-note">{cliente.notas}</div>}
                          </div>
                        </div>
                      </td>
                      ...
                      <td>
                        {deuda > 0 ? (
                          <span className="badge text-bg-danger">{formatearMoneda(deuda)}</span>
                        ) : (
                      ...
                      <td className="text-end">
                        <div className="d-flex justify-content-end gap-2 flex-wrap">
                          <button type="button" className="btn btn-outline-primary btn-sm" onClick={() => abrirEditarCliente(cliente)}>
                            <FaEdit className="me-2" /> Editar
                          </button>
                          <button type="button" className="btn btn-light btn-sm" onClick={() => abrirHistorial(cliente.id)}>
            ...
            <div className="custom-modal-header">
              <div>
                ...
                <p className="text-muted mb-0">
                  {clienteEditando ? "Actualiza la información del cliente." : "Ingresa la información del cliente."}
                </p>
              </div>
              <button type="button" className="modal-close" onClick={cerrarFormulario}><FaTimes /></button>
            </div>
              ...
              <div className="custom-modal-body">
                {!clienteEditando && (
                  <div className="mb-4">
                    ...
                    />
                    <button type="button" className="btn btn-outline-primary w-100" onClick={seleccionarVcard}>
                      <FaAddressBook className="me-2" />
                      {textoImportacion.boton}
                    </button>
                    <div className="text-muted small mt-2">{textoImportacion.ayuda}</div>
                  </div>
                )}
                ...
                <div className="row g-3">
                  <div className="col-md-6">
                    ...
                    <input type="text" name="nombre" className="form-control" value={formulario.nombre} onChange={cambiarCampo} required />
                  </div>
                  <div className="col-md-6">
                  ...
                  <div className="col-md-6">
                    ...
                    <input type="email" name="email" className="form-control" value={formulario.email} onChange={cambiarCampo} />
                  </div>
                  <div className="col-md-6">
                    <label className="form-label">Fecha de nacimiento</label>
                    <input type="date" name="fechaNacimiento" className="form-control" value={formulario.fechaNacimiento} onChange={cambiarCampo} />
                  </div>
                  <div className="col-12">
                    <label className="form-label">Notas</label>
                    <textarea name="notas" className="form-control" rows="3" value={formulario.notas} onChange={cambiarCampo} placeholder="Preferencias, observaciones..." />
                  </div>
                </div>
              </div>

              <div className="custom-modal-footer">
                <button type="button" className="btn btn-light" onClick={cerrarFormulario}>Cancelar</button>
          ...
          <div className="custom-modal custom-modal-large">
            {cargandoHistorial ? (
              ...
              <>
                <div className="custom-modal-header">
                  <div>
                    <h4>{clienteHistorial.cliente.nombre} {clienteHistorial.cliente.apellidos}</h4>
                    <p className="text-muted mb-0">Historial del cliente</p>
                  </div>
                  <button type="button" className="modal-close" onClick={() => setClienteHistorial(null)}><FaTimes /></button>
                </div>

                <div className="custom-modal-body">
                  <div className="row g-3 mb-4">
                    <div className="col-md-3"><div className="history-stat"><span>Total citas</span><strong>{clienteHistorial.resumen.totalCitas}</strong></div></div>
                    <div className="col-md-3"><div className="history-stat"><span>Completadas</span><strong>{clienteHistorial.resumen.citasCompletadas}</strong></div></div>
                    ...
                    <div className="col-md-3"><div className="history-stat"><span>Total gastado</span><strong>{formatearMoneda(clienteHistorial.resumen.totalGastado)}</strong></div></div>
                  </div>

                  {clienteHistorial.citas.length === 0 ? (
                    <div className="empty-state">Este cliente todavía no tiene citas.</div>
                  ) : (
                    <div className="table-responsive">
                      <table className="table align-middle">
                        <thead><tr><th>Fecha</th><th>Servicio</th><th>Profesional</th><th>Estado</th><th>Precio</th></tr></thead>
                        <tbody>
