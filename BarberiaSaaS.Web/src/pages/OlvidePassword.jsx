...
export default function OlvidePassword() {
  ...
  return <>
    ...
    <div className="auth-page"><div className="auth-card">
      <div className="auth-brand"><div className="auth-logo">BS</div><h2>Recuperar contraseña</h2><p>Te enviaremos un enlace seguro a tu correo.</p></div>
      {mensaje && <div className="alert auth-message auth-info mb-4">{mensaje}</div>}
      <form onSubmit={enviar}>
        <div className="mb-4"><label className="form-label auth-label">Correo electrónico</label><input type="email" className="form-control auth-input" value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="correo@ejemplo.com" required autoComplete="email" /></div>
        <button className="btn auth-button w-100" disabled={cargando}>{cargando ? "Enviando..." : "Enviar enlace"}</button>
      </form>
      <div className="auth-back"><Link to="/login">Volver a iniciar sesión</Link></div>
