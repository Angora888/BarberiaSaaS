...
public sealed class ExpoPushNotificationService:IPushNotificationService {
 ...
 public async Task EnviarTenantAsync(int tenantId,string titulo,string cuerpo,object? data=null,CancellationToken ct=default){
  try{
   var tokens=await _db.PushTokens.AsNoTracking().Where(x=>x.TenantId==tenantId&&x.Activo).Select(x=>x.Token).Distinct().ToListAsync(ct);
   ...
   if(!response.IsSuccessStatusCode)_logger.LogWarning("Expo Push respondió {StatusCode}.",response.StatusCode);
  }catch(Exception ex){_logger.LogError(ex,"No fue posible procesar Expo Push para tenant {TenantId}. La operación principal continuará.",tenantId);}
 }
}
