...
namespace BarberiaSaaS.Api.Controllers
{
    ...
    public class ConfiguracionController : ControllerBase
    {
        ...
        public async Task<IActionResult> Get()
        {
            ...
            var tenant = await _context.Tenants.Where(x => x.Id == tenantId && x.Activo)
                .Select(x => new
                {
            ...
            return tenant == null
                ? NotFound(new { mensaje = "Negocio no encontrado." })
                : Ok(tenant);
        }
        ...
        {
            var tenantId = _tenantContext.TenantId;
            var tenant = await _context.Tenants.Include(x => x.Configuracion)
                .FirstOrDefaultAsync(x => x.Id == tenantId && x.Activo);
            ...
            if (!EsColorHexValido(request.ColorPrimario) || !EsColorHexValido(request.ColorSecundario) || !EsColorHexValido(request.ColorFondo))
                return BadRequest(new { mensaje = "Los colores deben tener formato hexadecimal válido, por ejemplo #C62864." });
            if (request.DuracionSlotMinutos <= 0 || request.DuracionSlotMinutos > 120)
                return BadRequest(new { mensaje = "La duración del slot debe estar entre 1 y 120 minutos." });
            if (request.PorcentajeDeposito < 0 || request.PorcentajeDeposito > 100)
                return BadRequest(new { mensaje = "El porcentaje de depósito debe estar entre 0 y 100." });
            if (request.RecordatorioEmailHorasAntes < 1 || request.RecordatorioEmailHorasAntes > 168)
                return BadRequest(new { mensaje = "El recordatorio por correo debe programarse entre 1 y 168 horas antes." });
            ...
            configuracion.RecordatorioEmailActivo = request.RecordatorioEmailActivo;
            configuracion.RecordatorioEmailHorasAntes = request.RecordatorioEmailHorasAntes;
            ...
            var pendientes = await _context.Notificaciones.Where(x =>
                ...
                x.Estado != EstadosNotificacion.Cancelada).ToListAsync();
            ...
            {
                var citaIds = pendientes.Where(x => x.CitaId.HasValue).Select(x => x.CitaId!.Value).ToHashSet();
                var citas = await _context.Citas.Include(x => x.Cliente).Where(x =>
                    x.TenantId == tenantId &&
                    x.FechaInicio > DateTime.UtcNow &&
                    x.Estado != EstadosCita.Cancelada &&
                    x.Estado != EstadosCita.Completada &&
                    x.Estado != EstadosCita.NoAsistio).ToListAsync();
                ...
                foreach (var cita in citas)
                {
                    ...
                    if (email == null || programada <= DateTime.UtcNow)
                    {
                        if (existente != null) existente.Estado = EstadosNotificacion.Cancelada;
                        continue;
                    }
                    ...
                    if (existente != null)
                    {
                        ...
                        existente.ProgramadaPara = programada;
                        existente.Estado = EstadosNotificacion.Pendiente;
                        existente.Intentos = 0;
                        existente.UltimoError = null;
                    }
                    else if (!citaIds.Contains(cita.Id))
                    {
                        _context.Notificaciones.Add(new Notificacion
                        {
                            TenantId = tenantId, CitaId = cita.Id, ClienteId = cita.ClienteId,
                            Canal = CanalesNotificacion.Email, Tipo = TiposNotificacion.RecordatorioCita24H,
                            Destino = email, ProgramadaPara = programada, Estado = EstadosNotificacion.Pendiente,
