...
export function hoyRegional(r:Regional){try{const p=new Intl.DateTimeFormat("en-CA",{timeZone:r.zonaHoraria,year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(new Date());const v=(t:string)=>p.find(x=>x.type===t)?.value;return `${v("year")}-${v("month")}-${v("day")}`}catch{return new Date().toISOString().slice(0,10)}}
export function fechaCorta(v:string,r:Regional){const[y,m,d]=v.split("-").map(Number);return new Intl.DateTimeFormat(r.locale,{weekday:"short",day:"numeric",month:"short"}).format(new Date(y,m-1,d))}
export function horaCorta(v:string,r:Regional){const[h,m]=v.split(":").map(Number);return new Intl.DateTimeFormat(r.locale,{hour:"numeric",minute:"2-digit"}).format(new Date(2000,0,1,h,m))}
